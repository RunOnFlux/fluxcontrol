...
async function massFluxUpdate() {
  ...
  setTimeout(async () => {
    ...
    for (const ip of ips) {
      ...
      i++;
      if (fluxVersion !== "3.0.5" && fluxVersion !== "3.1.0" && fluxVersion !== "3.2.0" && fluxVersion !== "3.2.1") {
...
async function getApplicationLocations(application) {
  try {
    const response = await axiosGet(`https://api.runoflux.io/apps/location/${application}`, {
      timeout: 4567,
    });
    const ips = []
